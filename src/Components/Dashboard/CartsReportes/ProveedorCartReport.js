...
const CardReportes = () => {
  ...
  const generarReporte = async (event) => {
    ...
    try {
      const response = await api.get('/reportes/proveedor',{params:{
      ...
      link.href = url;
      link.setAttribute('download', 'Reporte de Proveedores.pdf'); 
      document.body.appendChild(link);
      link.click();

      Swal.fire({
        title: '¡Reporte generado!',
        text: 'El reporte de proveedores se ha generado correctamente.',
        icon: 'success',
      }).then(() => {
